...
def create_animated_chart(labels, data_values, chart_type="Barres horizontales", frame_duration=0.15):
    ...
    if chart_type == "Barres horizontales":
        ...
        for idx, series in enumerate(data_values):
            bar = ax.barh(labels, [0]*num_points, color=palette[idx], edgecolor='white', label=f"Série {idx+1}")
            bars.append(bar)
        ...
        bars = []
        for idx, series in enumerate(data_values):
            bar = ax.bar(labels, [0]*num_points, color=palette[idx], edgecolor='white', label=f"Série {idx+1}")
            bars.append(bar)
        ...
        for idx in range(num_series):
            ...
            value_texts = [ax.text(0, 0, '', fontsize=10, fontweight='bold', color='white') for _ in range(num_points)]
    ...
    for i in frames:
        if chart_type in ["Barres horizontales", "Barres verticales"]:
            ...
            for idx, series in enumerate(data_values):
                current_series = [val * i for val in series]
                collection = ax.fill_between(range(num_points), cumulative_data, cumulative_data + current_series, color=palette[idx], alpha=0.7)
