...
def create_animated_charts(labels, values, growth=None, chart_type_selection=None, frame_duration=0.15, progress_bar=None):
    charts = {}
...
def create_animated_chart(labels, values, growth=None, chart_type="Barres horizontales", frame_duration=0.15):
    try:
        ...
        fig, ax = plt.subplots(figsize=(8, 6))  # Taille fixe des figures
        ...
        if df.shape[1] < 2:
            ...
            if st.button("🎬 Générer les graphiques"):
                ...
                if not labels or not values:
                    ...
                    try:
                        charts = create_animated_charts(labels, values, growth, chart_type_selection, frame_duration, progress_bar)
                    except Exception as e:
